...
public class OfficeOrder implements Parcelable{
    ...
    protected OfficeOrder(Parcel in) {
        if (in.readByte() == 0) {
            mCustomerId = null;
        } else {
    ...
    public static final Creator<OfficeOrder> CREATOR = new Creator<OfficeOrder>() {
        @Override
        public OfficeOrder createFromParcel(Parcel in) {
            return new OfficeOrder(in);
        }

        @Override
        public OfficeOrder[] newArray(int size) {
            return new OfficeOrder[size];
        }
    };
