...
public class ShipToOrder implements Parcelable {
    ...
    protected ShipToOrder(Parcel in) {
        if (in.readByte() == 0) {
            mCustomerId = null;
        } else {
    ...
    public static final Creator<ShipToOrder> CREATOR = new Creator<ShipToOrder>() {
        @Override
        public ShipToOrder createFromParcel(Parcel in) {
            return new ShipToOrder(in);
        }

        @Override
        public ShipToOrder[] newArray(int size) {
            return new ShipToOrder[size];
        }
    };
