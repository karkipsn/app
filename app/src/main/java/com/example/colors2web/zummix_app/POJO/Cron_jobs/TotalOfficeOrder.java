...
public class TotalOfficeOrder implements Parcelable {
    ...
    protected TotalOfficeOrder(Parcel in) {
        if (in.readByte() == 0) {
            mNoOfOrders = null;
        } else {
    ...
    public static final Creator<TotalOfficeOrder> CREATOR = new Creator<TotalOfficeOrder>() {
        @Override
        public TotalOfficeOrder createFromParcel(Parcel in) {
            return new TotalOfficeOrder(in);
        }

        @Override
        public TotalOfficeOrder[] newArray(int size) {
            return new TotalOfficeOrder[size];
        }
    };
