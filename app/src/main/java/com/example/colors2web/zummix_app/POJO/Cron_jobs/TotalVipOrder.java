...
public class TotalVipOrder implements Parcelable {
    ...
    protected TotalVipOrder(Parcel in) {
        if (in.readByte() == 0) {
            mNoOfOrders = null;
        } else {
    ...
    public static final Creator<TotalVipOrder> CREATOR = new Creator<TotalVipOrder>() {
        @Override
        public TotalVipOrder createFromParcel(Parcel in) {
            return new TotalVipOrder(in);
        }

        @Override
        public TotalVipOrder[] newArray(int size) {
            return new TotalVipOrder[size];
        }
    };
