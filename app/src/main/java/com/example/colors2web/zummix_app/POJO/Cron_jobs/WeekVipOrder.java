...
public class WeekVipOrder implements Parcelable {
    ...
    protected WeekVipOrder(Parcel in) {
        if (in.readByte() == 0) {
            mCustomerId = null;
        } else {
    ...
    public static final Creator<WeekVipOrder> CREATOR = new Creator<WeekVipOrder>() {
        @Override
        public WeekVipOrder createFromParcel(Parcel in) {
            return new WeekVipOrder(in);
        }

        @Override
        public WeekVipOrder[] newArray(int size) {
            return new WeekVipOrder[size];
        }
    };
